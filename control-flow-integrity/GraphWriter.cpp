...
void ControlFlowIntegrityGraphPass::writeGraphFile(const graph::Graph &graph) {
	...
	for (auto &e : graph.getEdges()) {
		...
		if (std::find(paths.begin(), paths.end(), e.getDestination()) != paths.end() && std::find(paths.begin(), paths.end(), e.getOrigin()) != paths.end()) {
...
static RegisterPass<ControlFlowIntegrityGraphPass> X("control-flow-integrity", "Control Flow Integrity Pass", false, false);
